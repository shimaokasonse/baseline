...
count_vect_x = CountVectorizer(binary=True,tokenizer=lambda s: s.split(),lowercase=False)
count_vect_y = CountVectorizer(binary=True,tokenizer=lambda s: s.split(),lowercase=False)
...
#V = tf.Variable(tf.random_uniform([dim_h,dim_y], -0.001, 0.001))
#m = tf.matmul(u,V)
...
feed_hook = {x:X_hook, y:Y_hook,keep_prob:[1.0]}
for epoch in range(50):
    for i in range(2000):
        X_batch, Y_batch = sample_batch(X_train,Y_train,1000)
        feed = {x:X_batch, y:Y_batch,keep_prob:[0.5]}
        sess.run(optimizer, feed_dict = feed)
    #print("epoch:",epoch,"train:", sess.run(loss, feed_dict = feed_hook)/10000,"dev:", sess.run(loss, feed_dict = feed_dev)/X_dev.shape[0])
    print("=== TRAIN ===")
    acc_hook(sess.run(prediction,feed_dict={x:X_hook,keep_prob:[1.0]}),Y_hook)
    print("=== DEV ===")
    acc_hook(sess.run(prediction,feed_dict={x:X_dev,keep_prob:[1.0]}),Y_dev)
    print("=== TEST ===")
    acc_hook(sess.run(prediction,feed_dict={x:X_test,keep_prob:[1.0]}),Y_test)
    print("----")
